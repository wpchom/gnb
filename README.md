...
```shell
python3 gnb.py [build] [target] [-b <builddir>] [-p <profile>] [-o <outdir>] [--args <kv>] [-c] [-x <proxy>] [-v]
```
执行流程：
```shell
...
## 包管理
```shell
python3 gnb.py package <pkgname[:version]> [--download|--build|--list] [-c] [-r] [-t <target>] [--args <kv>] [-v] [-x <proxy>]
```
参数说明：
...
## 环境变量
- `MDS_GNB_PROXY`：网络代理（与 `-x/--proxy` 等效）。
- `MDS_REPO_DIR`：由工具在生成时注入，GN 脚本用于定位仓库路径。
